...
#sys.path.insert(0, '.../LDPKiT')
from transformer import MyTransformer
from text_modality_framework.process_data import sanitize_data, split_data, tokenize, process_data
from text_modality_framework.classifier import Classifier, model_performance, plot_loss_acc
from text_modality_framework.student_teacher import StudentTeacher
...
def plot_stuff(filenames, labels, name):
    ...
    datas = []
    for i in filenames:
        file = open(f"text_modality_framework/{i}/train_data.pkl", 'rb')
    ...
    plot(train_loss, "text_modality_framework/", f'{name} Student Validation Loss Curve')
    plot(student_acc, "text_modality_framework/", f'{name} Student Validation Accuracy Curve')
...
if __name__ == '__main__':
    file, st_args, te_args, config = args_and_init(config_file="text_modality_framework/config.yaml")
    ...
    if plot_: ## NEED TO CHANGE VARIABLES THIS PART FOR PLOTTING
        ...
        train = pd.read_csv('text_modality_framework/data/sentiment_data/huggingface_unseen69091.csv')
        local = pd.read_csv('text_modality_framework/data/sentiment_data/huggingface_private69092.csv')
        ...
        data = pd.read_csv('text_modality_framework/data/sentiment_data/huggingface_seen_pub252467.csv')
        ...
        tokenizer = BertTokenizer.from_pretrained(te_args['model'])
        dataname = 'text_modality_framework/data/sentiment_data/huggingface_seen_pub252467.csv'
        data = pd.read_csv(dataname)
        #local = pd.read_csv('text_modality_framework/data/sentiment_data/huggingface_private69092.csv')

        #print(f'Have sensitive dataset of size {len(local["label"])}')
        train_dataloader, valid_dataloader, test_data, weight = process_data(dataname, map, True, 
                                                                             te_args['hidden'][0],
                                                                             te_args['batch_size'], 
                                                                             RandomSampler,
                                                                             te_args['model'],
        ...
        # pre-train teacher
        start = time.time()
        pre_train(model=teacher, args=te_args, train_loader=train_dataloader,
                  valid_loader=valid_dataloader, test_data=test_data,

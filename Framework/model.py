...
class LSTMModelMulti2(nn.Module):
    def __init__(self, size, vocab_size, hidden, dropout):
        super(LSTMModelMulti2, self).__init__()
        self.embed = nn.Embedding(vocab_size, hidden[0], max_norm=True)
        self.dropout = nn.Dropout(dropout)
        self.lstm1 = nn.LSTM(hidden[0], hidden[1], bidirectional=True, batch_first = True)
        ...
        self.lstm4 = nn.LSTM(hidden[3]*2, hidden[4], bidirectional=True, batch_first = True)
        
        self.lstm5 = nn.LSTM(hidden[1]*2+hidden[3]*2+hidden[4]*2, hidden[5], bidirectional=True, batch_first = True)
